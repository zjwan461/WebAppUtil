...
public class IPUtil {
    ...
    public static String getAddrJsonFromIp(String IP) {
        InputStream resourceAsStream = IPUtil.class.getClassLoader().getResourceAsStream("env.properties");
        if (resourceAsStream == null) {
            throw new RuntimeException("no addr url found ...");
        }
        Properties prop = new Properties();
        String url = null;
        try {
            prop.load(resourceAsStream);
            url = prop.getProperty("addr.url");
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        String resout = "";
        try {
            String str = getJsonContent(url + IP);
